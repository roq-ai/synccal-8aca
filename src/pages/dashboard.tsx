...
import { getPlatformIntegrations } from 'apiSdk/platform-integrations';
import { PlatformIntegrationInterface } from 'interfaces/platform-integration';
import { getContentCategories } from 'apiSdk/content-categories';
import { ContentCategoryInterface } from 'interfaces/content-category';
import { getPostTemplates } from 'apiSdk/post-templates';
...
function DashboardPage() {
  ...
  const platformIntegrationStat = useSWR<PaginatedInterface<PlatformIntegrationInterface>>(
    () => `/platform-integrations`,
    () => getPlatformIntegrations({ limit: 1, offset: 0 }),
  );

  const contentCategoryStat = useSWR<PaginatedInterface<ContentCategoryInterface>>(
    () => `/content-categories`,
    () => getContentCategories({ limit: 1, offset: 0 }),
  );

  const postTemplateStat = useSWR<PaginatedInterface<PostTemplateInterface>>(
    () => `/post-templates`,
    () => getPostTemplates({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Platform Integrations',
      result: platformIntegrationStat?.data?.totalCount,
      isLoading: platformIntegrationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Content Categories',
      result: contentCategoryStat?.data?.totalCount,
      isLoading: contentCategoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Post Templates',
      result: postTemplateStat?.data?.totalCount,
      isLoading: postTemplateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
