...
function ScheduleViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ScheduleInterface>(
    ...
    () =>
      getScheduleById(id, {
        relations: ['content', 'social_media', 'user'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Schedule Date"
                  text={
                    data?.schedule_date ? format(parseISO(data?.schedule_date as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
                ...
                {hasAccess('social_media', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Social Media"
                    text={
                      <Link as={NextLink} href={`/social-medias/view/${data?.social_media?.id}`}>
