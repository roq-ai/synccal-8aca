...
export function ScheduleListPage(props: ScheduleListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getSchedules({
        relations: ['content', 'social_media', 'user', 'reminder.count'],
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.schedule_date ? format(parseISO(record?.schedule_date as unknown as string), 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/social-medias/view/${record.social_media?.id}`}
            >
              {record.social_media?.platform}
            </Link>
          ),
        }
      : null,
