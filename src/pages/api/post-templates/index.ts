...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPostTemplates() {
    ...
    const response = await prisma.post_template
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'post_template'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createPostTemplate() {
    ...
    const data = await prisma.post_template.create({
      data: body,
    });
