...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getContentCategories() {
    ...
    const response = await prisma.content_category
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'content_category'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createContentCategory() {
    await contentCategoryValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.content_category.create({
      data: body,
    });
